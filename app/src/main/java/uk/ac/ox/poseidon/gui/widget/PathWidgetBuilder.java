...
public class PathWidgetBuilder implements WidgetBuilder<JComponent, SwingMetawidget> {
    ...
    public JComponent buildWidget(String elementName, Map<String, String> attributes, SwingMetawidget metawidget) {
        ...
        if (actualClass == null || !Path.class.isAssignableFrom(actualClass)) return null;

        try {
            // nested address? no problem
            String address = StrategyFactoryWidgetProcessor.addressFromPath(attributes, metawidget);
            Path directory = ((Path) PropertyUtils.getProperty(metawidget.getToInspect(), address));

            JButton filer = new JButton();
            if (directory != null) filer.setText(directory.toString());

            final JFileChooser chooser = new JFileChooser();
            chooser.setFileSelectionMode(JFileChooser.FILES_AND_DIRECTORIES);
            if (directory != null) chooser.setCurrentDirectory(directory.toFile());
